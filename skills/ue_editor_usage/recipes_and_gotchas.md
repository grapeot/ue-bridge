...
有两种方案为卡牌等物体设置纹理材质。推荐使用方案一（Lit 材质 + 场景光照），更稳定可靠。

#### 方案一（推荐）：Lit 材质 + 场景光照

保持默认的 Lit ShadingModel，在场景中添加 DirectionalLight 提供基础照明。纹理连接到 BaseColor。这是 UE 标准渲染路径，行为稳定可预期。

```python
ue.create_material("M_Name")
# ShadingModel 默认就是 Lit，无需修改

# 创建 TextureSample 并设置纹理（两步法）
...
internal_name = summary["expressions"][0]["node_name"]
...
#### 方案二：Unlit 材质（不推荐用于生产）

将 ShadingModel 设为 Unlit，纹理连接到 EmissiveColor。不依赖光源但存在以下问题：

- 空关卡中 auto-exposure 会将 Emissive 输出过曝为纯白
- 截图捕获路径与实际显示差异更大
- 材质表现不符合 PBR 预期

仅在快速原型或测试中使用，生产环境应使用方案一。
...
使用 `Cube` 基础网格（不要用 `Plane`，Plane 面朝 +Z 上方，Yaw 旋转无法使其面向摄像机）。Scale 设为 `(width, 0.03, height)`，其中 Y 维度极薄作为厚度。Yaw 设为 90 度让薄面朝向摄像机。

扇形排列多张卡牌时，Yaw 偏移的方向决定了扇形的朝向：

- **扇形面向玩家（手牌效果）**：左侧卡牌 Yaw < 90（面稍偏右），右侧卡牌 Yaw > 90（面稍偏左），使卡面向内聚拢朝向玩家。例如：左卡 Yaw=75、中卡 Yaw=90、右卡 Yaw=105。
- **扇形面向对面**：Yaw 偏移方向相反，左卡 Yaw=105、右卡 Yaw=75。

常见错误是搞反偏移方向，导致扇形展开方向朝向对面玩家而非自己。
...
**`apply_material_to_actor` 是推荐的材质赋值方式**，不论是编辑器中还是 PIE 运行时都能正确工作。`set_static_mesh_properties` 的 `materials` 参数在 Blueprint CDO 上可能不生效（材质看起来设了但实际渲染时仍为默认材质）。

推荐流程：先 `spawn_blueprint_actor` 放置 Actor，再用 `apply_material_to_actor` 对每个实例应用材质，最后 `save_all`。
...
实测结论：`apply_material_to_actor` 设置的实例材质在 PIE 中也能正常工作（与之前文档中"PIE 可能仍使用默认材质"的描述不符）。将此方式作为主要材质赋值路径。
...
`spawn_blueprint_actor("BP_Foo", "MyActor")` 在编辑器中实际创建的是 `MyActor_UAID_XXXX`。注意：如果传入的名称包含数字后缀（如 `CardActor_0`），UAID 后缀直接拼在名称后面（如 `CardActor_UAID_XXXX`），数字后缀可能丢失或合并。因此不要依赖传入的名称做精确匹配。

推荐做法：使用不带数字后缀的前缀名（如 `Card_0`、`Card_1`），然后用 `get_actors()` + `startswith` 前缀匹配找到完整名称。如果多个 Actor 使用相同前缀，按 location 排序来区分。`find_actors()` 的模式匹配不可靠。

PIE 模式下例外：`get_pie_actors()` 返回的是干净的名称如 `Card_0`，`set_actor_transform()` 可以直接用这些名称。

### spawn_blueprint_actor 不应用 scale

`spawn_blueprint_actor` 的 scale 参数会被忽略。spawn 之后必须调用 `set_actor_transform()` 来设置缩放。

### set_static_mesh_properties 的 materials 参数不可靠

`set_static_mesh_properties(bp, component, static_mesh=..., materials=[...])` 的 `materials` 参数设置 Blueprint CDO 上的材质可能不生效。API 调用和编译都返回成功，但实际渲染时 Actor 仍使用默认材质（灰白格子图案）。

解决方式：先用 `set_static_mesh_properties` 设置网格（忽略 materials 参数或仅作为 fallback），然后对场景中的 Actor 实例用 `apply_material_to_actor` 逐个应用材质。
...
### take_screenshot 不可靠（macOS 尤甚）

截图捕获的是 viewport buffer，结果可能与用户在屏幕上看到的差异极大。在 macOS 上实测表现为：

- 空关卡（无背景）中截图可能**全白过曝**，而用户屏幕上能正常看到卡牌
- 有背景的关卡中截图可能**偏暗**
- `set_viewport_transform` + `select_actors` 强制刷新后仍然不可靠

这是 viewport 显示管线和截图捕获路径的 tonemapping/HDR 渲染差异。**不要依赖截图来验证材质、光照或场景是否正确**。应该直接在编辑器 viewport 中目视确认，或在 PIE 中检查 Actor 属性和日志。截图仅适合有成熟光照的完整场景。
...
### apply_material_to_actor 是推荐的材质赋值方式

`apply_material_to_actor` 设置的是关卡中 Actor 实例的材质覆盖。实测结论：

- **编辑器中有效**：viewport 中立即可见材质变化
- **PIE 中也有效**：实例的材质覆盖在 PIE spawn 时会被保留，与之前描述的"PIE 可能仍使用默认材质"不符
- **`set_static_mesh_properties` 的 `materials` 参数不可靠**：通过此方式设置 Blueprint CDO 上的材质可能不生效（编译通过，但渲染时仍为默认材质）

推荐流程：先 `spawn_blueprint_actor` 放置 Actor，再逐个用 `apply_material_to_actor` 设置材质，最后 `save_all`。不要依赖 `set_static_mesh_properties` 的 `materials` 参数作为主要材质赋值路径。
