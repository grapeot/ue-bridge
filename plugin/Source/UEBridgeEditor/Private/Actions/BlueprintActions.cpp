...
TSharedPtr<FJsonObject> FCreateBlueprintAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Created Blueprint '%s' with parent '%s'"),
...
UClass* FCreateBlueprintAction::ResolveParentClass(const FString& ParentClassName) const
{
	...
	// Fallback to Actor
	UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: Could not resolve parent class '%s', defaulting to AActor"),
		*ParentClassName);
	return AActor::StaticClass();
}
...
void FCreateBlueprintAction::CleanupExistingBlueprint(const FString& BlueprintName, const FString& PackagePath) const
{
	...
	if (ExistingPackage)
	{
		...
		{
			UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Blueprint '%s' exists in memory, cleaning up"), *BlueprintName);
	...
	{
		UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Blueprint '%s' exists on disk, deleting"), *BlueprintName);
...
{
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: compile_blueprint Validate called"));
	bool bResult = ValidateBlueprint(Params, Context, OutError);
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: compile_blueprint Validate result: %s, Error: '%s'"),
...
{
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: compile_blueprint ExecuteInternal called"));

	UBlueprint* Blueprint = GetTargetBlueprint(Params, Context);
	if (!Blueprint)
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: compile_blueprint - Blueprint not found"));
	...
	if (bSuccess)
	{
		...
		if (bIsWidgetBlueprint)
		{
			...
			{
				SavedPackagesCount++;
				UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Force-saved WBP GUID-patched package '%s'"), *Blueprint->GetName());
			}
		}
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Compiled Blueprint '%s' - Status: %s, Errors: %d, Warnings: %d"),
...
TSharedPtr<FJsonObject> FAddComponentToBlueprintAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Added component '%s' (%s) to Blueprint '%s'"),
...
TSharedPtr<FJsonObject> FSpawnBlueprintActorAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Spawned '%s' from Blueprint '%s' at (%f, %f, %f)"),
...
TSharedPtr<FJsonObject> FSetComponentPropertyAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Set property '%s' on component '%s' in Blueprint '%s'"),
...
bool FSetInheritedComponentPropertyAction::Validate(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context, FString& OutError)
{
	...
	FString Dummy;
	if (!GetRequiredString(Params, TEXT("component_name"), Dummy, OutError)) return false;
	if (!GetRequiredString(Params, TEXT("property_name"), Dummy, OutError)) return false;
	if (!Params->HasField(TEXT("property_value")))
	{
		OutError = TEXT("Missing 'property_value' parameter");
		return false;
...
TSharedPtr<FJsonObject> FSetInheritedComponentPropertyAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	if (!CDO)
	{
		return CreateErrorResponse(TEXT("Blueprint has no generated class or CDO"), TEXT("no_cdo"));
	}

	// Find the component on the CDO by iterating all components
	...
	if (ActorCDO)
	{
		...
		{
			if (Comp && (Comp->GetName() == ComponentName || Comp->GetName().Contains(ComponentName)))
			{
				FoundComponent = Comp;
				break;
			}
	...
	if (!FoundComponent)
	{
		...
		if (ActorCDO)
		{
			...
			for (UActorComponent* Comp : Components)
			{
				if (Comp) AvailableNames.Add(Comp->GetName());
			}
		}
		return CreateErrorResponse(
			FString::Printf(TEXT("Inherited component not found: %s. Available: %s"),
	...
	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	Result->SetStringField(TEXT("component"), FoundComponent->GetName());
	Result->SetStringField(TEXT("property"), PropertyName);
	Result->SetBoolField(TEXT("success"), true);
	return CreateSuccessResponse(Result);
}
...
TSharedPtr<FJsonObject> FSetStaticMeshPropertiesAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Set mesh properties on '%s' in Blueprint '%s'"),
...
TSharedPtr<FJsonObject> FSetPhysicsPropertiesAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Set physics properties on '%s' in Blueprint '%s'"),
		*ComponentName, *Blueprint->GetName());
...
TSharedPtr<FJsonObject> FSetBlueprintPropertyAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Set property '%s' on Blueprint '%s'"),
...
TSharedPtr<FJsonObject> FCreateColoredMaterialAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Created material '%s' with color (%.2f, %.2f, %.2f)"),
		*MaterialName, R, G, B);
