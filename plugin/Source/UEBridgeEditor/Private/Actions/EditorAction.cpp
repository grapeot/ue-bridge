...
TSharedPtr<FJsonObject> FEditorAction::Execute(const TSharedPtr<FJsonObject>& Params, FUEEditorContext& Context)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Action '%s' Execute started"), *GetActionName());

	// Step 1: Pre-validation
	if (!Validate(Params, Context, Error))
	{
		UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: Action '%s' validation failed: %s"), *GetActionName(), *Error);
		return CreateErrorResponse(Error, TEXT("validation_failed"));
	}

	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Action '%s' validation passed"), *GetActionName());

	// Step 2: Execute with crash protection
	TSharedPtr<FJsonObject> Result = ExecuteWithCrashProtection(Params, Context);
	if (!Result)
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Action '%s' returned nullptr!"), *GetActionName());
		return CreateCrashPreventedResponse();
	}

	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Action '%s' ExecuteInternal returned (has success=%s, has error=%s)"),
		*GetActionName(),
	...
	{
		UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: Action '%s' post-validation failed: %s"), *GetActionName(), *Error);
