...
void FUEEditorContext::SaveDirtyPackages()
{
	...
	for (UPackage* Package : DirtyPackages)
	{
		...
		{
			UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: SaveDirtyPackages - Could not resolve filename for '%s', skipping"), *PackageName);
			FailedCount++;
			continue;
		}
		...
		{
			UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: SaveDirtyPackages - Failed to save '%s'"), *PackageName);
			FailedCount++;
		}
	}
	...
	if (StillDirty.Num() > 0)
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: SaveDirtyPackages - %d packages still dirty after save:"), StillDirty.Num());
	...
	{
		UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: SaveDirtyPackages - Saved %d, Failed %d"), SavedCount, FailedCount);
	}
