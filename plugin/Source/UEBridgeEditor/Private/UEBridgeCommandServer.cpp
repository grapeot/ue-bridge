...
#include "UEBridgeCommandServer.h"
	...
	, bIsFinished(false)
{
	...
	Thread = FRunnableThread::Create(this, TEXT("UEBridgeEditor Client Handler"));
}
...
uint32 FEditorCommandClientHandler::Run()
{
	...
	while (!bShouldStop && !bServerStopping)
	{
		...
		{
			UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: Client connection timed out"));
			break;
		}
		...
		{
			UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Client disconnected or receive failed"));
			break;
		}
...
bool FEditorCommandClientHandler::ReceiveMessage(FString& OutMessage)
{
	...
	{
		UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: Invalid message length: %d"), Length);
		return false;
	}
...
{
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Client requested disconnect"));
...
bool FEditorCommandServer::Start()
{
	...
	if (!SocketSubsystem)
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Failed to get socket subsystem"));
		return false;
	}

	ListenerSocket = SocketSubsystem->CreateSocket(NAME_Stream, TEXT("UEBridgeEditor Listener"), false);
	if (!ListenerSocket)
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Failed to create listener socket"));
		return false;
	}
	...
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Failed to bind to port %d"), Port);
	...
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Failed to listen on socket"));
		SocketSubsystem->DestroySocket(ListenerSocket);
		ListenerSocket = nullptr;
		return false;
	}
	...
	bShouldStop = false;
	Thread = FRunnableThread::Create(this, TEXT("UEBridgeEditor Server Thread"));
	if (!Thread)
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Failed to create server thread"));
		SocketSubsystem->DestroySocket(ListenerSocket);
		ListenerSocket = nullptr;
		return false;
	}

	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Server started on port %d (max %d clients)"), Port, MaxClients);
	return true;
}
...
void FEditorCommandServer::Stop()
{
	...
	bIsRunning = false;
	bIsStopping = false;
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Server stopped"));
}
...
uint32 FEditorCommandServer::Run()
{
	...
	while (!bShouldStop)
	{
		...
		if (ListenerSocket->WaitForPendingConnection(bPendingConnection, FTimespan::FromSeconds(0.5)))
		{
			...
			{
				FSocket* ClientSocket = ListenerSocket->Accept(TEXT("UEBridgeEditor Client"));
				if (ClientSocket)
				{
					FScopeLock Lock(&HandlersLock);
					...
					{
						UE_LOG(LogMCP, Warning, TEXT("UEBridgeEditor: Max clients (%d) reached, rejecting connection"), MaxClients);
						ISocketSubsystem* SocketSubsystem = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);
						if (SocketSubsystem)
						{
							SocketSubsystem->DestroySocket(ClientSocket);
					...
					UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Client connected (total: %d)"), ClientHandlers.Num() + 1);
...
void FEditorCommandServer::CleanupFinishedHandlers()
{
	...
	for (int32 i = ClientHandlers.Num() - 1; i >= 0; --i)
	{
		...
		{
			UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Cleaning up finished client handler (remaining: %d)"), ClientHandlers.Num() - 1);
