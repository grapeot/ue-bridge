	...
	const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage)
{
	...
	// Support nested property paths (e.g., "NavAgentProps.bCanCrouch")
	...
	if (PropertyName.Contains(TEXT(".")))
	{
		...
		{
			FProperty* SegmentProp = CurrentStruct->FindPropertyByName(*PathSegments[i]);
			if (!SegmentProp)
			{
				OutErrorMessage = FString::Printf(TEXT("Property not found: %s (failed at segment '%s')"), *PropertyName, *PathSegments[i]);
				return false;
			}

			if (i == PathSegments.Num() - 1)
			{
				Property = SegmentProp;
				PropertyAddr = SegmentProp->ContainerPtrToValuePtr<void>(CurrentContainer);
			}
			else
			{
				...
				if (!StructProp)
				{
					OutErrorMessage = FString::Printf(TEXT("Intermediate property '%s' is not a struct"), *PathSegments[i]);
					return false;
				}
	...
	if (!PropertyAddr)
	{
		PropertyAddr = Property->ContainerPtrToValuePtr<void>(Object);
	}
