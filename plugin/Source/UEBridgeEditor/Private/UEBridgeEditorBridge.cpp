...
void UUEEditorBridge::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Bridge initializing"));
	...
	if (Server->Start())
	{
		UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Server started on port %d"), DefaultPort);
	}
	else
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Failed to start server"));
	}
}

void UUEEditorBridge::Deinitialize()
{
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Bridge deinitializing"));
...
void UUEEditorBridge::RegisterActions()
{
	...
	UE_LOG(LogMCP, Log, TEXT("UEBridgeEditor: Registered %d action handlers"), ActionHandlers.Num());
}
