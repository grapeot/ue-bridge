...
FString FEditorCommandExecutor::ExecuteGetContext(UUEEditorBridge* Bridge)
{
	...
	AsyncTask(ENamedThreads::GameThread, [Bridge, &Result, DoneEvent]()
	{
		...
		{
			UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Exception in ExecuteGetContext"));
...
FString FEditorCommandExecutor::ExecuteCommand(UUEEditorBridge* Bridge, const FString& CommandType, TSharedPtr<FJsonObject> Params)
{
	...
	AsyncTask(ENamedThreads::GameThread, [Bridge, CmdType = CommandType, Params, Result, DoneEvent, bCallerWaiting]()
	{
		...
		{
			UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Unhandled exception in ExecuteCommandSafe for '%s'"), *CmdType);
		}
	...
	{
		UE_LOG(LogMCP, Error, TEXT("UEBridgeEditor: Game thread execution timed out after %ds for command '%s'"),
